...
- **Subsequent rounds are delta-only against the matrix.** A reviewer may add a blocker only when the delta introduces a concrete regression or new evidence invalidates an accepted premise. Accepted follow-ups stay visible but cannot silently return to the merge gate.
