...
- **Regression tests are proven red-without-fix — against the dangerous mutant specifically** (see `_shared/engineering-discipline.md` §"Test Claims Name Their Layer"): revert the fix or apply the exact mutation the guard exists to catch, watch the new tests fail, restore. Assertions on source text are not assertions on behavior. Field-validated as the difference between "120 tests pass" and "these tests pin the contract"
